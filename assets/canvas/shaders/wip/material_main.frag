...
void main() {
	...
	**************************************/	
	...
	float nfc = 0.8;
	float nfx = frx_smootherstep(0.93, 1.0, six * sfaox * (1-bfx)) * nix * nfc;
	...
	**************************************/	
	...
	float ssa = frx_smootherstep(0.0, 1.0, fragData.light.y);
	...
	// blue fog, only indoor or at night
	a.b *= (1+max(0,bfx*(1-ssa)-dfx*ssa)*0.6);
	a.r *= (1+dfx*bfx*ssa*0.8);*/

	/**************************************
		END OLD BRIGHTNESS FOG CODE
	**************************************/	
